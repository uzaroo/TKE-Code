trigger CountNonPMOpenSATrigger on ServiceAppointment (after insert, after update, after delete) {
    ...
    Map<Id, WorkOrder> workOrders = new Map<Id, WorkOrder>([
        SELECT Id, 
               (SELECT Id, WorkTypeId, Status FROM ServiceAppointments) 
        FROM WorkOrder 
    ...
    for (Id workOrderId : workOrderCounts.keySet()) {
        WorkOrder wo = new WorkOrder(Id = workOrderId, TP_Count_Open_Non_PM_Appointments__c = workOrderCounts.get(workOrderId));
